...
// vendored mistlib node into the shared ConsumerClient so tc-books can send
// its LLM requests to a provider peer in a room instead of (or alongside)
// a direct OpenAI-compatible endpoint. Ported from tc-news's src/lib/network.ts
// (itself ported from tc-town) with the provider-hook re-exports dropped —
// tc-books is consumer-only.
 ...
 * event dispatcher and deinitialize the node out from under the rest of the app.
 *
...
class SharedMistNode implements MistNodeLike {
  ...
  joinRoom(roomId: string): void {
    ...
    this.joinPromise = this.realNode
      ? this.realNode.joinRoomAsync(roomId).catch((err: unknown) => {
          console.warn("tc-books: AI Network room join failed", err);
        })
      : null;
  }
  ...
  leaveRoom(): void {
    ...
    // the rest of the app still using it.
