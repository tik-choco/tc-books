...
import { loadEntries, loadCustomAccounts } from "./store";

const DONE_KEY = "tc-books:onboarding-done";
 ...
 * install. An existing install (entries or custom accounts already present
 * but no flag — i.e. a user from before onboarding shipped) is marked done
 * silently so they're never interrupted.
 */
export function shouldShowOnboarding(): boolean {
  if (isOnboardingDone()) return false;
  if (loadEntries().length > 0 || loadCustomAccounts().length > 0) {
    markOnboardingDone();
    return false;
  }
  return true;
